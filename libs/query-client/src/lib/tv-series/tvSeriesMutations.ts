  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      const shouldInvalidateChildren =
        oldSeriesLog?.status !== 'completed' && data.status === 'completed';
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueriesData(
        {
          predicate: ({ queryKey }) => {
            const refKey = tvSeasonLogOptions({
              userId: data.userId,
              tvSeriesId: data.tvSeriesId,
              seasonNumber: -1,
            }).queryKey;
            return queryKey[0] === refKey[0] && queryKey[1] === refKey[1] && queryKey[3] === 'log';
          },
        },
        null,
      );
      ...
      queryClient.setQueriesData(
        {
          predicate: ({ queryKey }) => {
            const refKey = tvEpisodeLogOptions({
              userId: data.userId,
              tvSeriesId: data.tvSeriesId,
              seasonNumber: -1,
              episodeNumber: -1,
            }).queryKey;
            return queryKey[0] === refKey[0] && queryKey[1] === refKey[1] && queryKey[4] === 'log';
          },
        },
        null,
      );
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (!oldUserTvSeriesLog) {
        ...
        queryClient.setQueryData(userTvSeriesLogKey, (old) => {
          if (!old) return undefined;
          return {
            ...oldUserTvSeriesLog,
            review: data,
