import {
  ...
  FeedControllerListInfiniteData,
...
export const userKeys = {
  ...
  profile: (params: { userId: string; username?: never } | { userId?: never; username: string }) =>
  ...
  follow: ({ userId, profileId }: { userId: string; profileId: string }) =>
  ...
  followRequests: ({
    ...
    | {
        mode: 'infinite';
        filters?: Omit<
          NonNullable<UserFollowRequestsControllerListInfiniteData['query']>,
          'cursor'
        >;
      }
  )) => {
  ...
  personFollow: ({ userId, personId }: { userId: string; personId: number }) =>
  ...
  playlistsFollowing: ({
    ...
    | {
        mode: 'infinite';
        filters?: Omit<
          NonNullable<MePlaylistsFollowingControllerListInfiniteData['query']>,
          'cursor'
        >;
      }
  )) => {
  ...
  playlistLike: ({ userId, playlistId }: { userId: string; playlistId: number }) =>
  ...
  playlistsSaved: ({
    ...
    | {
        mode: 'infinite';
        filters?: Omit<
          NonNullable<UserPlaylistsSavedControllerListInfiniteData['query']>,
          'cursor'
        >;
      }
  )) => {
  ...
  playlistSaved: ({ userId, playlistId }: { userId: string; playlistId: number }) =>
  ...
  playlistsAddTargets: ({
    ...
    | {
        mode: 'infinite';
        filters?: Omit<
          NonNullable<PlaylistsAddTargetsControllerListInfiniteData['query']>,
          'cursor'
        >;
      }
  )) => {
    const optionsKey = [...(mode !== undefined ? [mode] : []), ...(filters ? [filters] : [])];
    return [
      ...userKeys.details({ userId }),
      'playlists_add_targets',
      type,
      mediaId,
      ...optionsKey,
    ] as const;
  ...
  /* --------------------------------- Pinned --------------------------------- */
  pinned: ({ userId }: { userId: string }) => [...userKeys.details({ userId }), 'pinned'] as const,
  ...
  feed: ({
    ...
    | { mode?: never; filters?: never }
    | { mode: 'paginated'; filters?: NonNullable<UserFeedControllerListPaginatedData['query']> }
    | {
        mode: 'infinite';
