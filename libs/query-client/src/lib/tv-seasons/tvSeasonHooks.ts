...
import { LogTvSeasonUpdateResponse } from '@libs/api-js';
...
export const useTvSeasonLogCacheUpdate = () => {
  const queryClient = useQueryClient();
  const { updateLog } = useTvSeriesLogCacheUpdate();

  const setLog = useCallback(
    ({ season, series }: LogTvSeasonUpdateResponse) => {
      ...
      const shouldInvalidateEpisodes =
        oldSeasonLog?.status !== 'completed' && season.status === 'completed';
