...
export const FiltersBottomSheet = forwardRef<BottomSheetModal, FiltersBottomSheetProps>(
  ({ index, position }, ref) => {
    ...
    const [runtimeRange, setRuntimeRange] = useState<{ min: number; max: number } | undefined>(
      undefined,
    );
          ...
          >
            ...
            {user && (
              <View style={[tw`flex-row items-center justify-between`, { gap: GAP }]}>
                ...
                <Switch
                  key={`hide-watched-${filters.user.hideWatched}`}
                  value={filters.user.hideWatched}
                  onValueChange={(value) => setFilters({ user: { hideWatched: value } })}
                />
              </View>
            )}
          </BottomSheetScrollView>
