      ...
      <View style={tw`flex-row gap-4 shrink-0 items-start justify-between`}>
        ...
        <View style={tw`flex-1 gap-2`}>
          <View style={tw`flex-row items-center justify-between gap-4`}>
            {!skeleton ? (
              <Text style={tw`font-semibold`} numberOfLines={3}>
                {profile.name}
              </Text>
            ) : (
              <Skeleton style={tw`w-12 h-5`} />
            )}
            <View style={tw`flex-row items-center gap-4`}>
              {routesFollow.map((item, index) => (
                <Pressable key={index} style={tw`gap-0.5`} onPress={item.onPress}>
                  {!skeleton ? (
                    <Text style={tw`text-sm`}>{item.label}</Text>
                  ) : (
...
const ProfileScreen = () => {
  ...
  // Hooks
  const { data: widgetActivitiesMovie, isLoading: widgetActivitiesMovieLoading } = useInfiniteQuery(
    userMovieLogsInfiniteOptions({
      userId: profile?.id,
      filters: { sort_by: 'updated_at', sort_order: 'desc' },
    }),
  );
  const { data: widgetActivitiesTvSeries, isLoading: widgetActivitiesTvSeriesLoading } =
    useInfiniteQuery(
      userTvSeriesLogsInfiniteOptions({
        userId: profile?.id,
        filters: { sort_by: 'updated_at', sort_order: 'desc' },
  ...
  const { data: widgetPlaylists, isLoading: widgetPlaylistsLoading } = useInfiniteQuery(
    userPlaylistsInfiniteOptions({
      userId: profile?.id,
      filters: { sort_by: 'updated_at', sort_order: 'desc' },
    }),
  );
  const areWidgetsLoading = useMemo(
    () => widgetActivitiesMovieLoading || widgetActivitiesTvSeriesLoading || widgetPlaylistsLoading,
    [widgetActivitiesMovieLoading, widgetActivitiesTvSeriesLoading, widgetPlaylistsLoading],
  );
  ...
  return (
    <>
        ...
        options={{
          ...
          headerTitle: (props) => (
            <View style={tw`flex-row items-center gap-1`}>
              <HeaderTitle {...props}>{profile ? `@${profile.username}` : ''}</HeaderTitle>
          ...
          headerRight: () => (
            <>
              {(profile?.id === user?.id || user?.username === username) && (
                <Button
                  variant="ghost"
                  size="icon"
                  icon={Icons.settings}
      ...
      >
        ...
        {loading ? null : !profile?.isVisible ? (
