...
const TvSeriesSeasonHeader: React.FC<MediaHeaderProps> = ({
  ...
  const randomBg = useRandomImage(
    episodes?.filter((episode) => episode.stillPath).map((episode) => episode.stillPath!) ?? [],
  );
  // SharedValue
  ...
  return (
    ...
    >
      <Animated.View style={[tw`absolute inset-0`, bgAnim]}>
        {season && randomBg && (
          <Image
            source={{ uri: getTmdbImage({ path: randomBg, size: 'w1280' }) }}
      ...
      <Animated.View style={[tw`flex-row items-center gap-4`, textAnim]}>
        ...
        <Animated.View style={[{ gap: GAP }, tw`w-full`]}>
          {season ? (
            <Text>
          ...
          {season?.name && (
            <Text
              numberOfLines={1}
              style={[{ color: colors.mutedForeground }, tw`text-lg font-semibold`]}
            >
              {season.name}
            </Text>
          )}
...
const TvSeriesSeasonScreen = () => {
  ...
  const episodesLoading = useMemo(
    () => episodes === undefined || isEpisodesLoading,
    [episodes, isEpisodesLoading],
  );
  const flatEpisodes = useMemo(
    () => episodes?.pages.flatMap((page) => page.data) ?? [],
    [episodes],
  );
          ...
          <View style={tw`shrink px-2 py-1 gap-1`}>
            <Text numberOfLines={1}>
              <Text style={{ color: colors.accentYellow }}>
                {upperFirst(
                  t('common.messages.tv_episode_short', {
                    seasonNumber: season?.seasonNumber ?? 'N/A',
              ...
              <Text style={tw`font-bold`}>
                ...
                {item.name ??
                  upperFirst(
                    t('common.messages.tv_episode_value', { number: item.episodeNumber! }),
                  )}
              </Text>
            </Text>
            ...
            <Text numberOfLines={1} style={[tw`text-sm`, { color: colors.mutedForeground }]}>
              {item.airDate
                ? formatter.dateTime(new Date(item.airDate), {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                  })
  ...
  return (
    <>
      <AnimatedStackScreen
        options={{
          headerTitle: season
            ? `${season.tvSeries.name ?? ''} (${upperFirst(t('common.messages.tv_season_short', { number: season.seasonNumber }))})`
            : '',
        }}
        scrollY={scrollY}
