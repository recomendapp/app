...
const Filter = (props : NativeStackHeaderProps) => {
	...
	const routes = useMemo((): { type: SearchType; label: string; href: Href }[] => [
		{ type: 'all', label: upperFirst(t('common.messages.all', { count: 1, gender: 'male' })), href: { pathname: '/search', params: { q: search } } },
		{ type: 'movies', label: upperFirst(t('common.messages.film', { count: 2 })), href: { pathname: '/search/films', params: { q: search } } },
		{ type: 'tv_series', label: upperFirst(t('common.messages.tv_series', { count: 2 })), href: { pathname: '/search/tv-series', params: { q: search } } },
		{ type: 'persons', label: upperFirst(t('common.messages.person', { count: 2 })), href: { pathname: '/search/persons', params: { q: search } } },
		{ type: 'playlists', label: upperFirst(t('common.messages.playlist', { count: 2 })), href: { pathname: '/search/playlists', params: { q: search } } },
		{ type: 'users', label: upperFirst(t('common.messages.user', { count: 2 })), href: { pathname: '/search/users', params: { q: search } } },
	], [t, search]);
