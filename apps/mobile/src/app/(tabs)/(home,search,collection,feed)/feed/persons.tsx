import { CardFeedPersonMovie } from "apps/mobile/src/components/cards/feed/CardFeedPersonMovie";
import { CardFeedPersonTvSeries } from "apps/mobile/src/components/cards/feed/CardFeedPersonTvSeries";
...
const CastCrewFeedScreen = () => {
	...
	// Render
	const renderItem = useCallback(({ item } : { item: FeedPersonItem }) => {
		switch (item.type) {
			case 'movie':
	...
	const keyExtractor = useCallback((item: FeedPersonItem) => (
		`${item.media.id}:${item.type}-${item.person.id}`
	), []);
