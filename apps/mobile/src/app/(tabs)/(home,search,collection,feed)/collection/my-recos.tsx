...
const MyRecosScreen = () => {
  ...
  // Handlers
  const handleDeleteReco = useCallback(
    (data: RecoWithMedia) => {
      const title =
      ...
      Alert.alert(
        ...
        upperFirst(
          richTextToPlainString(
            t.rich('pages.collection.my_recos.modal.delete_confirm.description', {
              title: title,
  ...
  const bottomSheetActions = useMemo((): CollectionAction<RecoWithMedia>[] => {
    return [
      ...
      },
      {
        ...
        onPress: (item) =>
          openSheet(BottomSheetMyRecosSenders, {
  ...
  const swipeActions = useMemo(
    (): CollectionAction<RecoWithMedia>[] => [
      {
        icon: Icons.Delete,
        label: upperFirst(t('common.messages.delete')),
        onPress: handleDeleteReco,
