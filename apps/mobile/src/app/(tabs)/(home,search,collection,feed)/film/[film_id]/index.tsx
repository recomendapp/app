...
const FilmSynopsis = ({
  ...
  numberOfLines = 5,
  ...props
}: Omit<TextProps, 'children'> & { movie: Movie; containerStyle: ViewProps['style'] }) => {
  const t = useTranslations();
...
const FilmOriginalTitle = ({
  movie,
  style,
  numberOfLines = 1,
  ...props
}: Omit<TextProps, 'children'> & { movie: Movie }) => {
  const t = useTranslations();
  const { colors } = useTheme();

  if (!movie.originalTitle || lowerCase(movie.originalTitle) === lowerCase(movie.title!))
    return null;
...
const FilmTrailers = ({ movie }: { movie: Movie }) => {
  ...
  // States
  const [selectedTrailer, setSelectedTrailer] = useState<MovieTrailer | null>(
    movie.trailers?.at(0) || null,
  );
  const normalizedSite = useMemo(() => selectedTrailer?.site.toLowerCase(), [selectedTrailer]);
  // Render
  const renderItem = useCallback(
    ({ item }: { item: MovieTrailer }) => {
      const label =
        item.iso6391 === movie.originalLanguage ? 'VO' : item.iso6391?.toUpperCase() || 'N/A';
      return (
        <Button
