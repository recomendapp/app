...
import { BottomSheetSelectDate } from "apps/mobile/src/components/bottom-sheets/sheets/BottomSheetSelectDate";
...
const FilmWatchedDatesScreen = () => {
	...
	const handleAddWatchedDate = useCallback((date: Date) => {
		setWatchedDate({
			path: {
				movie_id: movieId,
			},
	...
	const handleUpdateWatchedDate = useCallback((date: WatchedDate) => {
		updateWatchedDate({
			path: {
				movie_id: movieId,
	...
	const handleSelectDate = useCallback((currentDate?: WatchedDate) => {
		openSheet(BottomSheetSelectDate, {
			...(currentDate ? {
				defaultDate: new Date(currentDate.watchedDate),
				onSave: (newDate) => handleUpdateWatchedDate({
					...currentDate,
	...
	const renderItem = useCallback(({ item }: { item: WatchedDate }) => {
		return (
		<Pressable onPress={() => handleSelectDate(item)} style={tw`py-2 px-4 border-b border-divider`}>
