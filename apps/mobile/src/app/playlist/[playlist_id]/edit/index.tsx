...
const ModalPlaylistEdit = () => {
  ...
  const posterOptions = useMemo(
    (): { label: string; value: 'library' | 'camera' | 'delete'; disable?: boolean }[] => [
      ...
      {
        label: upperFirst(t('common.messages.delete_current_image')),
        value: 'delete',
        disable: !playlist?.poster && !newPoster,
      },
    ],
    ...
    showActionSheetWithOptions(
      {
        ...
        disabledButtonIndices: posterOptions
          .map((option, index) => (option.disable ? index : -1))
          ...
          case 'camera': {
            ...
            if (!hasPermission.granted) {
              toast.error(upperFirst(t('common.messages.error')), {
                description: upperFirst(t('common.messages.camera_permission_denied')),
              });
              return;
            }
  ...
  return (
    <>
      ...
      <KeyboardAwareScrollView
        bounces={false}
        contentContainerStyle={[tw`gap-2 p-4`, { paddingBottom: insets.bottom + PADDING_VERTICAL }]}
        nestedScrollEnabled
        bottomOffset={headerHeight}
      >
        <Pressable
          onPress={handlePosterOptions}
          style={tw`relative items-center justify-center gap-2`}
        >
          {playlist ? (
            <ImageWithFallback
          ...
          <Text>
            {playlist?.poster
              ? upperFirst(t('common.messages.edit_image'))
              : upperFirst(t('common.messages.add_image'))}
          </Text>
        </Pressable>
        <Separator />
        <Controller
        ...
        {routes.length > 0 && (
          <>
            ...
            {routes.map((item, index) => (
              ...
              >
                <Button variant="ghost" size="fit" icon={item.icon}>
                  {item.label}
                </Button>
                <Button variant="ghost" icon={Icons.ChevronRight} size="icon" />
              </Pressable>
            ))}
          </>
        )}
