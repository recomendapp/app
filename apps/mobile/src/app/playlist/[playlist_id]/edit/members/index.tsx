...
import { usePlaylistMembers } from '../../../../../hooks/usePlaylistMembers';
...
const RightActions = ({
  ...
  const styleAnimation = useAnimatedStyle(() => {
    return {
      transform: [
        { translateX: (drag.value - PADDING_HORIZONTAL + actionWidth) * swipeActions.length },
      ],
    ...
    <Animated.View style={[tw`rounded-r-md overflow-hidden flex-row`, styleAnimation]}>
      {swipeActions.map((action, index) => (
        <Button
          ...
          variant={'ghost'}
          icon={action.icon}
          style={[tw`h-full rounded-none`, { width: actionWidth }]}
          size="icon"
          onPress={() => {
            action.onPress();
            swipeable.close();
...
const ModalPlaylistEditMembers = () => {
  ...
  const renderItem = useCallback(
    ({ item }: { item: PlaylistMemberWithUser }) => {
      return (
        <Swipeable
          friction={2}
        ...
        >
          <View style={tw`flex-row items-center justify-between gap-2 w-full`}>
            <CardUser
              user={item.user}
              linked={false}
              style={tw`border-0 p-0 h-auto bg-transparent`}
            />
        ...
        options={{
          ...
          headerRight: () => (
            <Button
              ...
              onPress={() =>
                router.push({
                  pathname: '/playlist/[playlist_id]/edit/members/add',
