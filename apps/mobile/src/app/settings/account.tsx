  ...
  const handleOnSubmit = useCallback(
    async (values: AccountFormValues) => {
      try {
        if (!user) return;
        setIsLoading(true);
        if (values.username !== user.username || values.private !== user.isPrivate) {
          await updateProfile(
            {
              body: {
          ...
          render={({ field: { onChange, onBlur, value } }) => (
            <View style={tw`gap-2`}>
              ...
              <Input
                ...
                rightComponent={
                  !form.formState.errors.username && value !== defaultValues.username ? (
                    isUsernameChecking ? (
                      <ActivityIndicator />
                    ) : (
                      <View
                        style={[
                          ...
                          tw`rounded-full h-6 w-6 items-center justify-center`,
                        ]}
                      >
                        {isUsernameAvailable ? (
              ...
              {usernameDisabled && (
                <Text style={[{ color: colors.destructive }, tw`text-right text-xs`]}>
                  {upperFirst(
                    t('common.messages.last_updated_at_date', {
                      date: format.dateTime(dateLastUsernameUpdate, {
                        dateStyle: 'long',
                        timeStyle: 'short',
                      }),
                    }),
