    ...
    >
      {bg && (
        <Animated.View entering={FadeIn.duration(2000)} style={tw`absolute inset-0`}>
          <Image source={{ uri: bg }} style={tw`absolute inset-0`} />
          <View style={tw`absolute inset-0 bg-black/50`} />
        </Animated.View>
      )}
      <Animated.Text
        entering={FadeInDown.duration(600).springify()}
        style={[{ color: colors.foreground }, tw`text-3xl font-bold text-center mb-4`]}
      >
        {upperFirst(t('pages.upgrade.subscription.success.label'))}
      </Animated.Text>
      <Button onPress={onClose}>{upperFirst(t('common.messages.lets_go'))}</Button>
    </Animated.View>
  );
};
    ...
    <>
      <Stack.Screen
        options={{
          ...
          unstable_headerLeftItems: isLiquidGlassAvailable
            ? (props) => [
                {
                  type: 'button',
