...
interface CardFeedPlaylistLikeBaseProps extends React.ComponentProps<typeof Animated.View> {
  variant?: 'default';
  onPress?: () => void;
  onLongPress?: () => void;
}
          ...
          {footer ||
            (!skeleton ? (
              data.content.description && (
                <Text numberOfLines={2} style={tw`text-xs text-justify`}>
                  {data.content.description}
                </Text>
              )
            ) : (
...
>(({ variant = 'default', onPress, onLongPress, ...props }, ref) => {
  ...
  const content =
    variant === 'default' ? <CardFeedPlaylistLikeDefault ref={ref} {...props} /> : null;
...
export { CardFeedPlaylistLike, CardFeedPlaylistLikeDefault };
