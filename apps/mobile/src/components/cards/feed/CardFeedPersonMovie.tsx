...
import { PersonFeedWithMovie } from '@libs/api-js';
import { FixedOmit } from '../../../utils/fixed-omit';

interface CardFeedPersonMovieBaseProps extends React.ComponentProps<typeof Animated.View> {
  variant?: 'default';
  onPress?: () => void;
  onLongPress?: () => void;
}
          ...
          {!skeleton ? (
            <Text textColor="muted">
              {t.rich('pages.feed.persons.new_activity', {
                ...
                titleMedia: data.media.title || t('common.messages.unknown'),
                linkPerson: (chunk) => (
                  <Link
                    href={{
                      pathname: '/person/[person_id]',
                ...
                linkMedia: (chunk) => (
                  <Link
                    href={{ pathname: '/film/[film_id]', params: { film_id: data.media.id } }}
                    style={{ color: colors.foreground }}
                  >
                    {chunk}
                  </Link>
...
export { CardFeedPersonMovie, CardFeedPersonMovieDefault };
