	...
	return (
		...
		>
			<View style={tw`flex-row items-center justify-center shrink`}>
				{!skeleton ? <UserAvatar full_name={user.name} avatar_url={user.avatar} /> : <UserAvatar skeleton />}
				<View style={tw.style('shrink px-2 py-1 gap-1')}>
					<View style={tw.style('flex-row items-center gap-1')}>
						{!skeleton ? <Text numberOfLines={2}>{user.name}</Text> : <Skeleton style={tw`w-20 h-5`} />}
						{!skeleton && user.isPremium && (
