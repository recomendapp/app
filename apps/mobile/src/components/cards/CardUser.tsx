...
interface CardUserBaseProps extends React.ComponentPropsWithoutRef<typeof Animated.View> {
  onPress?: () => void;
  onLongPress?: () => void;
...
export type CardUserProps = CardUserBaseProps & (CardUserSkeletonProps | CardUserDataProps);
      ...
      <View style={tw`flex-row items-center justify-center shrink`}>
        ...
        <View style={tw.style('shrink px-2 py-1 gap-1')}>
          <View style={tw.style('flex-row items-center gap-1')}>
            ...
            {!skeleton && user.isPremium && <Icons.premium color={colors.accentBlue} size={14} />}
          </View>
          {!skeleton ? (
            <Text numberOfLines={2} style={{ color: colors.mutedForeground }}>
              @{user.username}
            </Text>
          ) : (
            <Skeleton style={tw`w-20 h-5`} />
...
>(({ user, width, height, skeleton, children, style, ...props }, ref) => {
  ...
  return (
    <Animated.View
      ref={ref}
      style={[tw.style('flex-row items-center gap-1 font-bold'), style]}
      {...props}
    >
  ...
  return (
    <Animated.View style={[tw.style('flex-row items-center gap-1'), style]}>
      ...
      {!skeleton ? (
        ...
        <CardUserUsername skeleton {...props} />
      )}
    </Animated.View>
  );
});
...
>(({ user, skeleton, children, style, ...props }, ref) => {
  ...
  return (
    <Animated.View
      ref={ref}
      style={[tw.style('flex-row items-center justify-between p-1 gap-2'), style]}
      {...props}
    >
...
const CardUser = React.forwardRef<React.ComponentRef<typeof Animated.View>, CardUserProps>(
  (
    {
      variant = 'default',
      linked = true,
      onPress: onPressProps,
      onLongPress: onLongPressProps,
      containerStyle,
      ...props
    },
    ref,
    ...
    const onPress = React.useCallback(() => {
      ...
      onPressProps?.();
    }, [linked, onPressProps, props.user?.username, props.skeleton, router]);
    ...
    const Wrapper = linked || onPressProps || onLongPressProps ? Pressable : View;
...
export { CardUser, CardUserDefault };
