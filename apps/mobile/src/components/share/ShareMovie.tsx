...
const EditOptionsSelector = ({
  ...
  const renderEditItem = useCallback(
    (item: EditOption, isActive: boolean) => {
      return (
        <View
          style={[
...
export const ShareMovie = forwardRef<ShareViewRef, ShareMovieProps>(
    ...
    const posterUrl = useMemo(
      () =>
        poster
          ...
          : movie.posterPath
            ? getTmdbImage({ path: movie.posterPath, size: 'w342' })
            : undefined,
    ...
    const EditOptions = useMemo(() => {
      ...
      const content = (() => {
        switch (activeEditingOption) {
          case 'poster':
            return <PosterSelector movie={movie} poster={poster} setPoster={setPoster} />;
          case 'background':
