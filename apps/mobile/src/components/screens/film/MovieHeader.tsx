...
import BottomSheetMovieFollowersRating from '../../bottom-sheets/sheets/BottomSheetMovieFollowersRating';
...
const MovieHeader: React.FC<MovieHeaderProps> = ({ movie, loading, scrollY, triggerHeight }) => {
  ...
  const posterAnim = useAnimatedStyle(() => {
    ...
    return {
      transform: [{ translateY }, { scale: clampedScale }],
    };
  });
        ...
        {movie &&
          (movie.backdropPath ? (
            <AnimatedImage
              ...
              source={{ uri: getTmdbImage({ path: movie.backdropPath, size: 'w1280' }) ?? '' }}
            />
          ) : (
            palette &&
            palette.length > 1 && (
          ...
          <View>
            {!loading ? (
              <Text
                variant="title"
                numberOfLines={2}
                style={[tw`text-center`, !movie && !loading && { color: colors.mutedForeground }]}
              >
...
const Directors = ({
  directors,
  ...props
}: Omit<TextProps, 'children'> & { directors: PersonCompact[] }) => {
  ...
  const onPress = useCallback(
    (person: PersonCompact) => {
      router.push({
        ...
        params: { person_id: person.slug || person.id },
      });
    },
    [router],
  );
        ...
        if (part.type === 'element') {
          return (
            <Text
              ...
              onLongPress={() => onLongPress(director!)}
            >
              {director?.name}
            </Text>
          );
