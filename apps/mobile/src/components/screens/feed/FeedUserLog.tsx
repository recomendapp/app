...
interface FeedUserLogProps extends React.ComponentProps<typeof View> {
  author: UserSummary;
  log: LogMovie | LogTvSeries;
}

const FeedUserLog = forwardRef<React.ComponentRef<typeof View>, FeedUserLogProps>(
  ({ author, log, style, ...props }, ref) => {
    const { colors } = useTheme();
    const t = useTranslations();
    return (
      <View ref={ref} style={[tw`flex-row items-center gap-2`, style]} {...props}>
        {log.review ? (
          <>
            <Text>
              {t.rich('pages.feed.actions.reviewed', {
                name: () => <Text style={tw`font-semibold`}>{author.username}</Text>,
              })}
            </Text>
          </>
        ) : (
