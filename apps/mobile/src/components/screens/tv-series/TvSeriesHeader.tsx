...
const TvSeriesHeader: React.FC<TvSeriesHeaderProps> = ({
  ...
  return (
    <Animated.View
      style={[tw.style('w-full'), { paddingTop: navigationHeaderHeight }]}
    ...
    >
        ...
        {tvSeries &&
          (tvSeries.backdropPath ? (
            <AnimatedImage
              ...
              source={{ uri: getTmdbImage({ path: tvSeries.backdropPath, size: 'w1280' }) ?? '' }}
            />
          ) : (
            palette &&
            palette.length > 1 && (
      ...
      >
        ...
        <Animated.View style={[tw.style('w-full items-center'), { gap: GAP }, textAnim]}>
          ...
          {tvSeries ? (
            <TvSeriesHeaderInfo tvSeries={tvSeries} />
          ) : loading ? (
            <Skeleton style={tw.style('w-32 h-8')} />
          ) : null}
        </Animated.View>
      </Animated.View>
    </Animated.View>
