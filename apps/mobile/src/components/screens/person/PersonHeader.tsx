...
export const PersonHeader: React.FC<PersonHeaderProps> = ({
  ...
  const { palette } = useImagePalette(
    getTmdbImage({ path: person?.profilePath, size: 'w92' }) || undefined,
  );
  // SharedValue
  ...
  return (
    ...
    >
      <Animated.View style={[tw`absolute inset-0`, bgAnim]}>
        {palette?.length ? (
          <View style={[{ backgroundColor: palette[0] }, tw`absolute inset-0`]} />
        ) : null}
        <LinearGradient
      ...
      >
        ...
        <Animated.View style={[tw`gap-2 w-full`, textAnim]}>
          {/* GENRES */}
          {person ? (
            <Text>
