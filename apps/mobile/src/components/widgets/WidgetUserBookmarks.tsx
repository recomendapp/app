...
export const WidgetUserBookmarks = ({
  ...
  const { user } = useAuth();
  const { data: watchlist } = useInfiniteQuery(
    userBookmarksInfiniteOptions({
      userId: user?.id,
      filters: {
        sort_by: 'random',
      },
  ...
  const flattendWatchlist = (watchlist?.pages.flatMap((page) => page.data) || []).slice(
    0,
    MAX_ITEMS,
  );
  ...
  return (
    <View style={[{ gap: GAP }, style]}>
      ...
      <View style={containerStyle}>
        <GridView data={flattendWatchlist} renderItem={(item) => <WatchlistItem item={item} />} />
      </View>
    </View>
  );
};
