...
interface BottomSheetMovieFollowersRatingProps extends BottomSheetProps {
  movieId: number;
}
...
>(({ id, movieId, detents = [0.8], ...props }, ref) => {
  ...
  const { user } = useAuth();
  const { data: followersRating, isLoading } = useQuery(
    movieFollowingLogsOptions({
      userId: user?.id,
      movieId: movieId,
    }),
  );
  ...
  return (
    <TrueSheet ref={ref} detents={detents} scrollable {...props}>
      <FlatList
        data={followersRating}
        renderItem={({ item }) => (
          <CardUser user={item.user!} style={tw`h-auto`}>
            <IconMediaRating rating={item.rating} variant="follower" />
          </CardUser>
        )}
        ListHeaderComponent={
          chartsData ? (
...
BottomSheetMovieFollowersRating.displayName = 'BottomSheetMovieFollowersRating';

export default BottomSheetMovieFollowersRating;
