...
import ButtonActionPlaylistLike from '../../buttons/ButtonActionPlaylistLike';
import ButtonActionPlaylistSaved from '../../buttons/ButtonActionPlaylistSaved';
  ...
  const items = useMemo<Item[]>(
    () => [
      ...additionalItemsTop,
      {
        icon: Icons.Share,
        onPress: () =>
          openSheet(BottomSheetSharePlaylist, {
            playlist: playlist,
          }),
        ...
        ? [
            {
              ...
              onPress: toggle,
              label: isSaved
                ? upperFirst(t('common.messages.remove_from_library'))
                : upperFirst(t('common.messages.save_to_library')),
              closeSheet: false,
            },
          ]
        : []),
        ...
        ? [
            {
              icon: Icons.User,
              onPress: () =>
                router.push({ pathname: '/user/[username]', params: { username: owner.username } }),
            ...
            >
              <View style={[tw`flex-row items-center`, { gap: GAP }]}>
                <ImageWithFallback
                  ...
                  style={[{ aspectRatio: 1 / 1 }, tw.style('rounded-md w-12')]}
                  type={'playlist'}
                />
                <View>
                  <Text numberOfLines={2}>{playlist.title}</Text>
                  {owner && (
                    <Text textColor="muted" numberOfLines={1}>
