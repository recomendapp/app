...
>(({ id, path, contentRef, children, ...props }, ref) => {
    ...
    (): SharePlatform[] => [
      ...
      },
      {
        ...
        onPress: async () => {
          ...
          toast.success(
            upperFirst(t('common.messages.downloaded', { count: downloaded, gender: 'female' })),
          );
        },
      },
      {
  ...
  const handlePlatformPress = useCallback(
    (item: SharePlatform, index: number) => async () => {
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: SharePlatform; index: number }) => (
      <Button
        variant="ghost"
        size="fit"
