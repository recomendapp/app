...
interface BottomSheetMovieProps extends BottomSheetProps {
  movie: MovieCompact;
  log?: LogMovieWithMovieNoReview & { user: UserSummary };
...
>(({ id, movie, log, additionalItemsTop = [], additionalItemsBottom = [], ...props }, ref) => {
  ...
  const items: Item[] = React.useMemo(
    () => [
      ...
      ...additionalItemsTop,
      {
        icon: Icons.Share,
        onPress: () =>
          openSheet(BottomSheetShareMovie, {
            movie: movie,
          }),
        ...
        ? [
            {
              ...
              onPress: () =>
                router.push({
                  ...
                  params: { username: log.user.username, film_id: movie.id },
                }),
          ...
          >
            <View style={tw`flex-row items-center gap-2 `}>
              <ImageWithFallback
                ...
                style={[{ aspectRatio: 2 / 3, height: 'fit-content' }, tw.style('rounded-md w-12')]}
                type={'movie'}
              />
  ...
  return (
    <TrueSheet ref={ref} scrollable {...props}>
      ...
      />
      {movie.directors && (
        <BottomSheetDefaultView ref={BottomSheetMainCreditsRef} id={`${id}-credits`} scrollable>
          <FlashList
