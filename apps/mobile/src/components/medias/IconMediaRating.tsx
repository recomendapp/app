...
interface IconMediaRatingProps extends React.ComponentPropsWithRef<typeof Animated.View> {
...
>(({ rating, variant = 'general', skeleton, style, ...props }, ref) => {
  ...
  if (skeleton) {
    return <Skeleton style={[{ aspectRatio: 3 / 2 }, tw`relative w-8`, style]} />;
  }
  ...
  return (
    <Animated.View
      ...
      style={[
        ...
        tw.style(
          'relative flex shadow-sm w-8 rounded-sm border-2 justify-center items-center shrink-0',
        ),
        style,
      ]}
      {...props}
    >
      <Animated.Text style={[{ color: variantStyles.text }, tw`font-bold`]}>
