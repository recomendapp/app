...
type CollectionHeaderProps = CollectionHeaderBaseProps &
  (CollectionHeaderLoadedProps | CollectionHeaderLoadingProps);
  ...
  (
    ...
    return (
      ...
      >
        ...
        >
          {bgBackdrop && (
            <Image transition={500} style={tw`absolute inset-0`} source={bgBackdrop} />
          )}
          <LinearGradient
        ...
        >
          <View style={tw`items-center justify-center`}>
            ...
            {!hideTitle &&
              (!loading ? (
                <Text
                  numberOfLines={2}
                  style={[{ color: colors.accentYellow }, tw`text-4xl font-bold text-center`]}
                >
                  {title}
                </Text>
              ) : (
                <Skeleton style={tw`h-10 w-48`} />
              ))}
            {!hideNumberOfItems &&
              (!loading ? (
          ...
          {bottomText ? (
            typeof bottomText === 'function' ? (
              ...
              <Text numberOfLines={1}>{bottomText}</Text>
            )
          ) : null}
        </Animated.View>
      </Animated.View>
