...
export class WebhookRevenuecatService {
  ...
  async handleEvent(event: any) {
    ...
    const validUuids = [...new Set(potentialIds)].filter(
      (id: string) => z.uuid().safeParse(id).success,
    );
  ...
  private async syncSingleUser(userId: string) {
    ...
    await this.db.update(profile).set({ isPremium }).where(eq(profile.id, userId));
