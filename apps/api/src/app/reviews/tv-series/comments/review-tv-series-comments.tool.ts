...
const reviewIdParameter = z.number().int().describe('The id of the TV series review');
...
@McpController()
export class ReviewTvSeriesCommentsTool {
  constructor(private readonly reviewTvSeriesCommentsService: ReviewTvSeriesCommentsService) {}

  @Tool({
    name: 'comment-tv-series-review',
    description: 'Comment on a TV series review, or reply to a top-level comment of it',
  ...
  async commentTvSeriesReview(
    @Payload() { reviewId, ...dto }: z.infer<typeof commentParameters>,
    @McpRawRequest() request: McpAuthenticatedRequest,
  ) {
    const comment = await this.reviewTvSeriesCommentsService.create({
      user: request.user,
      reviewId,
      dto,
    });
  ...
  @Tool({
    name: 'list-tv-series-review-comments',
    description:
      'List the top-level comments of a TV series review. ' +
      'Use "list-tv-series-review-comment-replies" for the replies of a comment.',
  ...
  async listTvSeriesReviewComments(
    @Payload()
    { reviewId, sortBy, sortOrder, page, perPage }: z.infer<typeof listCommentsParameters>,
    @McpRawRequest() request: McpAuthenticatedRequest,
  ) {
    const comments = await this.reviewTvSeriesCommentsService.listPaginated({
      reviewId,
  ...
  @Tool({
    name: 'list-tv-series-review-comment-replies',
    description: 'List the replies to a top-level comment of a TV series review',
  ...
  async listTvSeriesReviewCommentReplies(
    @Payload()
    {
      reviewId,
      commentId,
    ...
    const replies = await this.reviewTvSeriesCommentsService.listRepliesPaginated({
      reviewId,
      commentId,
      query: { sort_by: sortBy, sort_order: sortOrder, page, per_page: perPage },
      currentUser: request.user,
  ...
  @Tool({
    name: 'update-tv-series-review-comment',
    description: 'Edit a comment the current user wrote on a TV series review',
  ...
  async updateTvSeriesReviewComment(
    @Payload() { reviewId, commentId, body }: z.infer<typeof updateCommentParameters>,
    @McpRawRequest() request: McpAuthenticatedRequest,
  ) {
    const comment = await this.reviewTvSeriesCommentsService.update({
      user: request.user,
      reviewId,
      commentId,
      dto: { body },
  ...
  @Tool({
    name: 'delete-tv-series-review-comment',
    description: 'Delete a comment the current user wrote on a TV series review',
  ...
  async deleteTvSeriesReviewComment(
    @Payload() { reviewId, commentId }: z.infer<typeof deleteCommentParameters>,
    @McpRawRequest() request: McpAuthenticatedRequest,
  ) {
    const comment = await this.reviewTvSeriesCommentsService.delete({
      user: request.user,
      reviewId,
      commentId,
    });
