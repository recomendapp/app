...
import { profile, reviewTvSeriesComment } from '@libs/db/schemas';
import {
...
describe('ReviewTvSeriesCommentLikesService', () => {
  ...
  it('likes a comment, increments the trigger-maintained count, and notifies its author', async () => {
    ...
    const { review } = await createTestReviewTvSeries(testDb.db, { userId: reviewAuthor.id });
    ...
    const service = new ReviewTvSeriesCommentLikesService(testDb.db, notify);
  ...
  it('rejects liking a deleted comment', async () => {
    ...
    const { user: liker } = await createTestUser(testDb.db);
    const { review } = await createTestReviewTvSeries(testDb.db, { userId: author.id });
    const comment = await createTestReviewTvSeriesComment(
      testDb.db,
      { reviewId: review.id, userId: author.id },
      { deletedAt: new Date().toISOString() },
    );

    const service = new ReviewTvSeriesCommentLikesService(testDb.db, createFakeNotifyClient());
