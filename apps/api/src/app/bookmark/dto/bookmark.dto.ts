import { ApiSchema, ApiProperty, PartialType, PickType, getSchemaPath, ApiPropertyOptional, ApiExtraModels } from '@nestjs/swagger';
...
export enum BookmarkStatusFilter {
  ACTIVE = 'active',
  COMPLETED = 'completed',
  ALL = 'all',
}
...
@ApiSchema({ name: 'GetBookmarksQuery' })
export class GetBookmarksQueryDto extends PaginationQueryDto {
    @ApiPropertyOptional({
      enum: BookmarkStatusFilter,
      default: BookmarkStatusFilter.ACTIVE,
    })
    @IsOptional()
    @IsEnum(BookmarkStatusFilter)
    status?: BookmarkStatusFilter = BookmarkStatusFilter.ACTIVE;
