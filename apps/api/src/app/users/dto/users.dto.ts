...
export class UserDto {
  ...
  @Expose()
  @Transform(({ value }) =>
    typeof value === 'string' ? USER_RULES.USERNAME.normalization(value) : value,
  )
  @IsString()
  @Length(USER_RULES.USERNAME.MIN, USER_RULES.USERNAME.MAX)
  @Matches(USER_RULES.USERNAME.REGEX, {
    message:
      'Invalid username format (3-15 characters, letters, numbers, dots, no consecutive dots)',
  })
  username!: string;
  ...
  @ApiProperty({
    ...
    description: 'The URL of the user background image',
    nullable: true,
  })
  @Expose()
  ...
  @ApiProperty({
    description:
      'Timestamp indicating when the user completed the post-signup welcome flow, null if not yet completed',
    nullable: true,
  })
  @Expose()
  @IsDateString()
...
export class ProfileDto extends PickType(UserDto, [
  ...
  @ApiProperty({
    description: 'Whether the user profile is visilble by current user (if target user is private)',
  })
  @Expose()
  isVisible!: boolean;
}
...
export class UpdateUserDto extends PartialType(
  PickType(UserDto, ['name', 'username', 'bio', 'isPrivate', 'language'] as const),
) {
  @ApiPropertyOptional({
    description: 'Set to true to mark the post-signup welcome flow as completed',
  })
  @IsOptional()
  @IsBoolean()
  welcomed?: boolean;
