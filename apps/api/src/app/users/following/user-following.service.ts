    ...
    const orderBy = (() => {
      switch (sortBy) {
        ...
        case UserSortBy.FOLLOWERS_COUNT:
          return [direction(profile.followersCount), direction(follow.followingId)];
        case UserSortBy.CREATED_AT:
        default:
          return [direction(follow.createdAt), direction(follow.followingId)];
      }
    })();

    const whereClause = and(eq(follow.followerId, targetUserId), eq(follow.status, 'accepted'));
      ...
      switch (sort_by) {
        ...
        default: {
          ...
          cursorWhereClause = or(
            ...
            and(eq(follow.createdAt, createdDate), operator(follow.followingId, cursorData.id)),
          );
          break;
        }
      }
