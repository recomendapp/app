...
export class FeedService {
  ...
  private buildFeedContentSql({
    ...
    const movieFeedJsonb = buildJsonbObject({
      ...MOVIE_SUMMARY_SELECT,
      overview: tmdbMovieView.overview,
    });
    const tvSeriesFeedJsonb = buildJsonbObject({
      ...TV_SERIES_SUMMARY_SELECT,
  ...
  private async getFeedBaseWhere(
    ...
    if (targetUserId) {
      ...
      if (!currentUser)
        throw new UnauthorizedException('User must be logged in to view their feed');

      const followingIdsSubquery = tx
        .select({ followingId: follow.followingId })
        .from(follow)
        .where(and(eq(follow.followerId, currentUser.id), eq(follow.status, 'accepted')));

      conditions.push(
        or(eq(feed.userId, currentUser.id), inArray(feed.userId, followingIdsSubquery)),
      );
    }
  ...
  async listPaginated({
    ...
    query: ListPaginatedFeedQueryDto;
    currentUser: User | null;
  ...
  async listInfinite({
    ...
    query: ListInfiniteFeedQueryDto;
    currentUser: User | null;
    locale: SupportedLocale;
    targetUserId?: string;
    ...
    return await this.db.transaction(async (tx) => {
      ...
      if (cursorData) {
        finalWhere = and(
          baseWhere,
          sql`(${feed.createdAt}, ${feed.id}) < (${String(cursorData.value)}, ${Number(cursorData.id)})`,
        );
      }
