...
export class MoviePlaylistsService {
  ...
  private getListBaseWhereClause(tx: DbTransaction, movieId: number, currentUser: User | null) {
    const visibilityCondition = PlaylistQueryBuilder.getVisibilityCondition(tx, currentUser);

    const containsMovieCondition = exists(
      tx
    ...
    return and(containsMovieCondition, visibilityCondition);
  }
  ...
  async listPaginated({
    ...
    return await this.db.transaction(async (tx) => {
      ...
      const baseWhereClause = this.getListBaseWhereClause(tx, movieId, currentUser);
