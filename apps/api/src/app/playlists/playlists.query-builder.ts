...
export class PlaylistQueryBuilder {
    ...
    switch (sortBy) {
      ...
      default:
        return or(
          ...
          and(
            eq(playlist.createdAt, String(cursorData.value)),
            operator(playlist.id, cursorData.id),
          ),
        );
    }
  ...
  static getNextCursorValue(
    lastItem: typeof playlist.$inferSelect,
    sortBy: PlaylistSortBy,
  ): string | number {
    switch (sortBy) {
      case PlaylistSortBy.LIKES_COUNT:
        return lastItem.likesCount ?? 0;
      case PlaylistSortBy.UPDATED_AT:
  ...
  static getVisibilityCondition(
    ...
    if (staticOwner) {
      followerLogic = staticOwner.isFollowingTarget
