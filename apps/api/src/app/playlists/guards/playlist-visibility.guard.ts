...
import { OptionalAuthenticatedRequest } from '../../auth/types/fastify';
...
export class PlaylistVisibilityGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request = context
      .switchToHttp()
      .getRequest<OptionalAuthenticatedRequest & { params: { playlist_id: string } }>();

    const user = request.user || null;
    const playlistIdStr = request.params.playlist_id;
    ...
    const playlistRecord = await this.db.query.playlist.findFirst({
      where: and(eq(playlist.id, playlistId), visibilityCondition),
      columns: { id: true },
    });
