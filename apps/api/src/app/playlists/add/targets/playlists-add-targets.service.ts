...
export class PlaylistsAddTargetsService {
  ...
  private getListBaseQuery(
    ...
    const isOwnerPremium = exists(
      this.db
        ...
        .from(profile)
        .where(and(eq(profile.id, playlist.userId), eq(profile.isPremium, true))),
    );
    ...
    const savedPlaylistIdsSubquery = this.db
      .select({ id: playlistSaved.playlistId })
      .from(playlistSaved)
  ...
  async listInfinite({
    ...
    const {
      ...
      orderBy,
    } = this.getListBaseQuery(currentUser, type, mediaId, sort_by, sort_order, filter, search);
