...
describe('PlaylistItemsService', () => {
  ...
  describe('get', () => {
    it('throws when the item does not exist', async () => {
      ...
      await expect(
        service.get({ playlistId: p.id, itemId: 999999, locale: defaultSupportedLocale }),
    ...
    it('does not leak an item belonging to another playlist', async () => {
      ...
      await expect(
        service.get({ playlistId: p2.id, itemId: item.id, locale: defaultSupportedLocale }),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('update', () => {
    it('throws when the item does not exist and nothing is being updated', async () => {
      ...
      await expect(
        service.update({ playlistId: p.id, itemId: 999999, dto: {} as never }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws when the item does not exist and a comment update was requested', async () => {
      ...
      await expect(
        service.update({ playlistId: p.id, itemId: 999999, dto: { comment: 'hi' } as never }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('touches the playlist updatedAt when an actual field changes', async () => {
      ...
      await service.update({ playlistId: p.id, itemId: item.id, dto: { comment: 'touched' } });
    ...
    it('returns the existing item unchanged (and does not touch updatedAt) when the dto is empty', async () => {
      ...
      const result = await service.update({ playlistId: p.id, itemId: item.id, dto: {} as never });
    ...
    describe('repositioning', () => {
      it('moves an item to position 1, before the current first item', async () => {
        ...
        await service.update({ playlistId: p.id, itemId: c.id, dto: { position: 1 } as never });
      ...
      it('inserts an item between its two new neighbors', async () => {
        ...
        await service.update({ playlistId: p.id, itemId: c.id, dto: { position: 2 } as never });
      ...
      it('moves an item to the end when only one neighbor remains before it', async () => {
        ...
        await service.update({ playlistId: p.id, itemId: a.id, dto: { position: 3 } as never });
      ...
      it('clamps a position far beyond the end to simply the last slot', async () => {
        ...
        await service.update({ playlistId: p.id, itemId: a.id, dto: { position: 999 } as never });
    ...
    it('broadcasts an item-updated event to playlist recipients', async () => {
      ...
      await service.update({ playlistId: p.id, itemId: item.id, dto: { comment: 'updated' } });
  ...
  describe('delete', () => {
    it('returns an empty array and does nothing when given no ids', async () => {
      ...
      const result = await service.delete({ playlistId: p.id, dto: { itemIds: [] } });
    ...
    it('does not delete items from another playlist', async () => {
      ...
      const result = await service.delete({ playlistId: p2.id, dto: { itemIds: [item.id] } });
    ...
    it('decrements the playlist items_count via the counter trigger', async () => {
      ...
      await service.delete({ playlistId: p.id, dto: { itemIds: [a.id] } });
