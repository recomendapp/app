    ...
    return await this.db.transaction(async (tx) => {
      ...
      const [results, totalCount] = await Promise.all([
        tx
          ...
          .leftJoin(
            tmdbMovieView,
            and(eq(mediaMostPopular.mediaId, tmdbMovieView.id), eq(mediaMostPopular.type, 'movie')),
          )
          .leftJoin(
            tmdbTvSeriesView,
            and(
              eq(mediaMostPopular.mediaId, tmdbTvSeriesView.id),
              eq(mediaMostPopular.type, 'tv_series'),
            ),
          )
          .orderBy(...orderBy)
          .limit(per_page)
