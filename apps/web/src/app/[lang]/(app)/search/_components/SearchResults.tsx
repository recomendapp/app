...
const SearchBestResult = ({
  ...
  return (
    <div className={cn('flex flex-col gap-2', className)}>
      {data ? (
        <h2 className="text-2xl font-bold">{upperFirst(t('common.messages.top_result'))}</h2>
      ) : (
      ...
      {data ? (
        data?.type === 'movie' ? (
          <CardMovie
            variant="row"
            movie={data.data}
            className="max-w-[600px] h-80"
            posterClassName="h-full w-auto"
          />
        ) : data?.type === 'tv_series' ? (
          <CardTvSeries
            variant="row"
...
const SearchMovies = ({
  ...
  return (
    <div className={cn('flex flex-col gap-2', className)}>
      {data ? (
        <Button variant={'link'} className="text-2xl font-bold justify-start p-0" asChild>
