...
const TermsOfUsePage = async () => {
  ...
  const data = [
    ...
    },
    {
      ...
      title: t('sections.child_safety.label'),
      content: t.rich('sections.child_safety.content', {
        app: siteConfig.name,
        email: (chunks) => (
          <Link
            href="mailto:help@recomend.app"
    ...
    },
    {
      ...
      content: t.rich('sections.law.content', {
        email: (chunks) => (
          <Link
    ...
    <div className="flex flex-col gap-4 items-center text-justify transition-all">
      {data.map((section) => (
        <section
          key={section.section}
          id={section.section}
          className="w-full flex flex-col gap-4 px-4 max-w-xl scroll-mt-20"
        >
          {section.title && (
