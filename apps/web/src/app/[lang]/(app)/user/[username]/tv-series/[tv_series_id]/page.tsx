...
export async function generateMetadata(props: {
  ...
  return {
    title: upperFirst(
      t('pages.user.metadata.title', { full_name: profile.name, username: profile.username }),
    ),
    description: truncate(
      upperFirst(
        t('pages.user.metadata.description', { username: profile.username!, app: siteConfig.name }),
      ),
    ...
    openGraph: {
      ...
      description: truncate(
        upperFirst(
          t('pages.user.metadata.description', {
...
export default async function UserMovie(props: {
  params: Promise<{
    lang: SupportedLocale;
    username: string;
    tv_series_id: string;
  ...
  const jsonLd: WithContext<Review> = {
    ...
    '@type': 'Review',
    name: t('pages.review.metadata.title', {
      title: log.tvSeries.name!,
      username: log.user.username,
    }),
    description: log.review
      ? truncate(removeMd(log.review.body), { length: siteConfig.seo.description.limit })
      : undefined,
    ...
    itemReviewed: {
      ...
      image: log.tvSeries.posterPath
        ? getTmdbImage({ path: log.tvSeries.posterPath, size: 'w500' })
        : undefined,
