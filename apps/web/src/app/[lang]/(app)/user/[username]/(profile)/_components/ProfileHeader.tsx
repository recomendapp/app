...
export const ProfileHeader = ({ profile: profileServer }: { profile?: Profile | null }) => {
  const { user } = useAuth();
    ...
    <HeaderBox
      ...
      background={
        profile?.backgroundImage
          ? { src: profile.backgroundImage, alt: profile.username ?? '' }
          : undefined
      }
    >
            ...
            <ButtonGroup className="@lg/header-box:hidden">
              <ButtonGroup>
                <ProfileFollowersButton
                  userId={profile?.id}
                  disabled={!profile?.isVisible ? true : false}
                />
                <ProfileFolloweesButton
                  userId={profile?.id}
                  disabled={!profile?.isVisible ? true : false}
                />
              </ButtonGroup>
