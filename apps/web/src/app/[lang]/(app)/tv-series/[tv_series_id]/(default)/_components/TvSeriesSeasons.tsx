...
export const TvSeriesSeasons = ({ tvSeries }: { tvSeries: TvSeries }) => {
  const t = useTranslations();

  const { data, isLoading } = useQuery(
    tvSeriesSeasonsOptions({
      ...
      <div className="flex space-x-4 pb-4">
        {isLoading
          ...
          : data?.map((season, i) => (
              <Link
                key={i}
                href={`/tv-series/${tvSeries.slug || tvSeries.id}/season/${season.seasonNumber}`}
              >
                <Card
                  className={cn(
                    'flex flex-col gap-2 h-full w-32 p-2 hover:bg-muted-hover',
                    season.seasonNumber === 0 ? 'opacity-70' : '',
                  )}
                >
                  ...
                  <div className="text-center">
                    <p className="line-clamp-2 wrap-break-word">
                      {season.seasonNumber !== 0
                        ...
                        : upperFirst(t('common.messages.tv_special_episode', { count: 2 }))}
                    </p>
