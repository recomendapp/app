...
export const TvSeasonHeader = ({ season }: { season: TvSeasonGet }) => {
  const { user } = useAuth();
  const t = useTranslations();
  const title = upperFirst(t('common.messages.tv_season_value', { number: season.seasonNumber }));
  const { data: episodes } = useInfiniteQuery(
    ...
    <div>
      <HeaderBox
        ...
        background={
          randomBg
            ? {
                src: getTmdbImage({ path: randomBg.src, size: 'w1280' }),
                alt: randomBg.alt ?? '',
                unoptimized: true,
              }
            : undefined
        ...
        <div className="w-full max-w-7xl flex flex-row gap-4 items-center">
          ...
          >
            <div className="absolute flex flex-col gap-2 top-2 right-2 w-12">
              {season.voteAverage ? (
                <IconMediaRating rating={season.voteAverage} variant="general" className="w-full" />
              ) : null}
            </div>
          </MediaPoster>
          <div className="flex flex-col justify-between gap-2 w-full h-full py-4">
            ...
            <div>{t('common.messages.tv_episode_count', { count: season.episodeCount })}</div>
          </div>
        </div>
      </HeaderBox>
      {user ? (
        <div className="flex flex-col items-center">
          <div className="max-w-7xl w-full flex justify-between gap-2 px-4 pb-4">
            <div className="flex gap-2 overflow-x-auto items-center">
              <ButtonLogTvSeasonRating tvSeason={season} />
              <ButtonLogTvSeasonWatch tvSeason={season} />
            </div>
          </div>
        </div>
      ) : null}
