...
const sortBySchema = z.enum(['updated_at', 'created_at', 'likes_count', 'rating', 'random']);
...
export const TvSeriesReviews = ({ tvSeries }: { tvSeries: TvSeries }) => {
...
const MyReviewButton = ({ tvSeries }: { tvSeries: TvSeries }) => {
  const t = useTranslations();
  const { user } = useAuth();
  ...
  return (
    <Button variant={'outline'} asChild>
      <Link
        href={
          activity?.review
            ? `/@${user.username}/tv-series/${tvSeries.slug || tvSeries.id}`
            : `/tv-series/${tvSeries.slug || tvSeries.id}/review`
        }
      >
