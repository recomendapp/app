...
export async function generateMetadata(props: {
  ...
  return {
    title: t('pages.tv_series.metadata.title', {
      title: tvSeries.name!,
    ...
    description: truncate(
      tvSeries.createdBy
        ? t('pages.tv_series.metadata.description', {
            title: tvSeries.name!,
            creators: new Intl.ListFormat(lang, { style: 'long', type: 'conjunction' }).format(
              tvSeries.createdBy.map((creator) => creator.name ?? ''),
            ),
            year: new Date(String(tvSeries.firstAirDate)).getFullYear(),
...
export default async function TvSeriesPage(props: {
  ...
  const jsonLd: WithContext<TVSeries> = {
    ...
    image: tvSeries.posterPath
      ? getTmdbImage({ path: tvSeries.posterPath, size: 'w500' })
      : undefined,
    description: tvSeries.overview ?? undefined,
    datePublished: tvSeries.firstAirDate ?? undefined,
    dateModified: new Date().toISOString(),
    director: tvSeries.createdBy?.map((director) => ({
      '@type': 'Person',
