...
export async function generateMetadata(props: {
  ...
  return {
    ...
    description: truncate(
      t('pages.person.tv_series.metadata.description', { name: person.name! }),
    ...
    openGraph: {
      ...
      url: `${siteConfig.url}/person/${person.slug}/tv-series`,
...
export default async function TvSeriesPage(props: {
  params: Promise<{
    lang: SupportedLocale;
    person_id: string;
  }>;
  ...
  const [facets, person] = await Promise.all([getPersonTvSeriesFacets(id), getPerson(lang, id)]);
  ...
  const tvSeries = await getPersonTvSeries(id, {
    page: page,
    sort_by: sortBy,
