...
export async function generateMetadata(props: {
  ...
  return {
    ...
    description: truncate(t('pages.person.films.metadata.description', { name: person.name! }), {
    ...
    openGraph: {
      ...
      url: `${siteConfig.url}/person/${person.slug}/films`,
...
export default async function PersonFilmsPage(props: {
  params: Promise<{
    lang: SupportedLocale;
    person_id: string;
  }>;
  ...
  const [facets, person] = await Promise.all([getPersonFilmsFacets(id), getPerson(lang, id)]);
  ...
  const department = getValidateDepartment(
    facets.departments.map((d) => d.department),
    searchParams.department,
  );
  const job = getValidateJob(facets.departments, department, searchParams.job);
  const movies = await getPersonFilms(id, {
    page: page,
    sort_by: sortBy,
    ...
    <div className="flex flex-col items-center">
      <div className="@container/person-films flex flex-col gap-4 max-w-7xl w-full">
        <div className="space-y-2">
          ...
          <ActiveFilters department={department} job={job} />
        </div>
        <div
          className={` gap-2
					${
