...
export const Filters = ({
  ...
  const handleChange = useCallback(
    (entries: { [key: string]: string | null }, resetPage = true) => {
  ...
  return (
    <ButtonGroup>
                    ...
                    return (
                      <SelectItem key={i} value={department.department}>
                        ...
                        {department.department === knownForDepartment && (
                          <Icons.star
                            size={14}
                            className="inline text-accent-yellow fill-accent-yellow ml-2"
                          />
                        )}
                      </SelectItem>
                    );
      ...
      <ButtonGroup>
        <TooltipBox
          tooltip={upperFirst(
            sortOrder === 'asc' ? t('common.messages.order_asc') : t('common.messages.order_desc'),
          )}
        >
          <Button
            variant={'outline'}
            onClick={() => handleChange({ sort_order: sortOrder === 'asc' ? 'desc' : 'asc' })}
          >
