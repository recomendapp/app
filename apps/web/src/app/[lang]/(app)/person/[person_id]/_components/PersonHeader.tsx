...
import {
  personMoviesInfiniteOptions,
  personOptions,
  personTvSeriesInfiniteOptions,
} from '@libs/query-client';

export const PersonHeader = ({ person: initialPerson }: { person: Person }) => {
  ...
  const { data: tvSeries } = useInfiniteQuery(
    personTvSeriesInfiniteOptions({ personId: person.id }),
  );
  ...
  const randomBg = useRandomImage([
    ...
    ...(tvSeries?.pages.flatMap((page) =>
      page.data.map(({ tvSeries }) => ({
      ...
      background={
        randomBg
          ? {
              ...
              alt: randomBg.alt || `${person.slug}-backdrop`,
              unoptimized: true,
            }
          : undefined
      }
        ...
        <div className="flex flex-col justify-between gap-2 w-full h-full py-4">
          ...
          <div>
            ...
            {person.knownForDepartment && (
              <span className=" before:content-['_|_']">{person.knownForDepartment}</span>
            )}
          </div>
          {/* NAME */}
