...
export const WidgetPersonFilms = ({ personId, personSlug }: WidgetPersonFilmsProps) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      <Link
        href={`/person/${personSlug}/films`}
        className={cn(buttonVariants({ variant: 'link' }), 'font-semibold text-xl p-0 w-fit')}
      >
        ...
        <div className="flex space-x-4 pb-4">
          {isLoading
            ? Array.from({ length: DEFAULT_PER_PAGE }).map((_, i) => (
                <Skeleton
                  key={i}
                  className="w-24 lg:w-32 rounded-md aspect-2/3"
                  style={{ animationDelay: `${i * 0.12}s` }}
                />
              ))
            : movies.map(({ movie }, i) => (
                <CardMovie key={i} variant="poster" movie={movie} className="w-24 lg:w-32" />
              ))}
          {isFetching ? <Icons.loader /> : <div ref={ref} />}
        </div>
