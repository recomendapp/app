...
export function PlaylistHeader({
  ...
  return (
    <ContextMenuPlaylist playlist={playlist}>
      <HeaderBox
        background={
          !skeleton && backdrop?.src
            ? {
                src: getTmdbImage({ path: backdrop.src, size: 'w1280' }),
                alt: 'Likes Header Background',
                unoptimized: true,
              }
            : undefined
        }
          ...
          <div className="flex flex-col justify-between gap-2 w-full">
            ...
            <div>
              <span className="text-accent-yellow">
                {upperFirst(t('common.messages.playlist', { count: 1 }))}
              </span>
              <span className=" before:content-['_|_']">
                {playlist.visibility !== 'public'
                  ? upperFirst(t('common.messages.private', { gender: 'female', count: 1 }))
                  : upperFirst(t('common.messages.public'))}
              </span>
            </div>
            <div>
              <h1
