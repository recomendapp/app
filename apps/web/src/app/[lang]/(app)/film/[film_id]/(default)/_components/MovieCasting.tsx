...
export const MovieCasting = ({ movie }: { movie: Movie }) => {
        ...
        {isLoading
          ...
          : data?.map(({ person, roles }, i) => (
              <Link
                key={i}
                href={{
                  pathname: `/person/${person.slug || person.id}`,
                }}
              >
                <Card className="flex flex-col gap-2 h-full w-32 p-2 hover:bg-muted-hover">
                  ...
                  <div className="text-center">
                    ...
                    {roles && roles.length > 0 ? (
                      <p className="line-clamp-2 text-accent-yellow italic text-sm">
                        {roles.map((role) => role.character).join(', ')}
                      </p>
                    ) : null}
                  </div>
                </Card>
