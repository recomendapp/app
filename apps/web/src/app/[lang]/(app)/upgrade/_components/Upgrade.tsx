...
const GetStarted = ({
  ...
  if (!offering) {
    ...
    if (customerInfo?.entitlements.active['premium']) {
      return (
        <Button className="w-full" asChild>
          <Link href={`/settings/subscription`}>{upperFirst(t('common.messages.manage'))}</Link>
        </Button>
      );
    } else {
      return (
        <Button className="w-full" onClick={onPurchase}>
          {upperFirst(
            t('common.messages.upgrade_to_plan', {
...
export const Upgrade = () => {
  const { user, customerInfo } = useAuth();
  const { isLoading: isOfferingLoading, offering, purchasePackage } = useOffering();
    ...
    (): PlanFeature[] => [
      {
        ...
        features: [
          {
            name: upperFirst(
              t('pages.upgrade.subscription.features.activities.unlimited_activities.label'),
            ),
            description: upperFirst(
              t('pages.upgrade.subscription.features.activities.unlimited_activities.description'),
            ),
            free: true,
            premium: true,
          },
          {
            ...
            description: upperFirst(
              t('pages.upgrade.subscription.features.activities.favorites.description'),
            ),
            free: true,
            premium: true,
            soon: true,
      ...
      },
      {
        ...
        features: [
          {
            name: upperFirst(
              t('pages.upgrade.subscription.features.playlists.unlimited_playlists.label'),
            ),
            description: upperFirst(
              t('pages.upgrade.subscription.features.playlists.unlimited_playlists.description'),
            ),
            free: true,
            premium: true,
          },
          {
            name: upperFirst(
              t('pages.upgrade.subscription.features.playlists.shared_playlists.label'),
            ),
            description: upperFirst(
              t('pages.upgrade.subscription.features.playlists.shared_playlists.description'),
            ),
            free: true,
            premium: true,
          },
          {
            name: upperFirst(
              t('pages.upgrade.subscription.features.playlists.collaborative_playlists.label'),
            ),
            description: upperFirst(
              t(
                'pages.upgrade.subscription.features.playlists.collaborative_playlists.description',
              ),
            ),
            free: false,
            premium: true,
        ...
        features: [
          {
            ...
            description: upperFirst(
              t('pages.upgrade.subscription.features.feed.followees.description'),
            ),
            free: true,
            premium: true,
          },
          {
            ...
            description: upperFirst(
              t('pages.upgrade.subscription.features.feed.cast_and_crew.description'),
            ),
            free: false,
            premium: true,
          },
      ...
      },
      {
        ...
        features: [
          {
            name: upperFirst(
              t('pages.upgrade.subscription.features.support.team.label', {
                app: siteConfig.name,
              }),
            ),
            free: '+',
          ...
          },
          {
            ...
            description: upperFirst(
              t('pages.upgrade.subscription.features.support.credits.description'),
            ),
            free: false,
            premium: true,
            soon: true,
                  ...
                  premium: formatPrice(
                    {
                      unit_amount:
                        billingInterval === 'annual'
                          ? (offering?.annual?.webBillingProduct.price.amountMicros || 0) / 10_000
                          : (offering?.monthly?.webBillingProduct.price.amountMicros || 0) / 10_000,
                      currency: offering?.monthly?.webBillingProduct.price.currency,
                    },
                    locale,
                  ),
  ...
  const handleSelectPlan = useCallback(() => {
    ...
    openModal(ModalSubscription, {
      offering: offering,
      preselectedPlan:
        (billingInterval === 'annual' ? offering.annual : offering.monthly) || undefined,
        ...
        <div className="container flex flex-col w-full gap-4 justify-center max-w-xl mx-0 my-10">
          ...
          {!isOfferingLoading ? (
            offering && (
              <h2>
                {upperFirst(
                  t('pages.upgrade.header.subtitle', {
                    price: `${formatPrice({ unit_amount: (offering?.monthly?.webBillingProduct.price.amountMicros || 0) / 10_000, currency: offering?.monthly?.webBillingProduct.price.currency }, locale)}`,
                  }),
                )}
              </h2>
            )
          ...
          <div className="flex gap-4 max-w-xs">
            ...
            <Button variant={'outline'} className="w-full" asChild>
              <Link href="#features">{upperFirst(t('common.messages.compare_plans'))}</Link>
            </Button>
          </div>
        </div>
      </HeaderBox>
                ...
                {offering && (
                  <>
                    ...
                    <Switch
                      ...
                      onCheckedChange={() =>
                        setBillingInterval(billingInterval === 'annual' ? 'monthly' : 'annual')
                      }
                    />
                      ...
                      {offering && (
                        <span className="absolute -top-10 start-auto -end-28">
                          <span className="flex items-center">
                            ...
                            <Badge className="mt-3 uppercase shrink-0">
                              {t('common.messages.save_up_to_percent', {
                                percent: calculateSave(
                                  {
                                    unit_amount:
                                      offering?.annual?.webBillingProduct.price.amountMicros || 0,
                                    interval: 'yearly',
                                  },
                                  {
                                    unit_amount:
                                      offering?.monthly?.webBillingProduct.price.amountMicros || 0,
                                    interval: 'monthly',
                                  },
                                ),
                              })}
                ...
                {featureType.features.map((feature, i) => (
                  <TableRow key={i}>
                    <TableCell>
                      {feature.name}{' '}
                      {feature.soon && (
                        <span className="text-xs text-accent-yellow">{`(${t('common.messages.soon')})`}</span>
                      )}
                      {feature.description && (
