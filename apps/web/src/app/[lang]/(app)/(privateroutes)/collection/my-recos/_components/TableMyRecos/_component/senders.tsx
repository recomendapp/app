...
export default function Senders({ row }: { row: Row<RecoWithMedia> }) {
  ...
  return (
    ...
    >
      <AvatarGroup>
        {row.original.senders
          ?.slice(0, senderToShow)
          .reverse()
          .map((item, i) => (
            <div key={i} className="relative">
              {item.user?.username ? (
                <UserAvatar avatarUrl={item.user.avatar} username={item.user.username} />
              ) : null}
              {item?.comment ? (
                <Text
                  size={15}
