...
interface FeedPersonsItemProps extends React.ComponentProps<typeof Card> {
  item: FeedPersonItem;
}

const FeedPersonsItemDefault = forwardRef<HTMLDivElement, FeedPersonsItemProps>(
  ({ className, item, ...props }, ref) => {
    ...
    return (
      <Card
        ref={ref}
        className={cn('@container/feed-item flex flex-row gap-4 p-2', className)}
        {...props}
      >
        <div
            ...
            <div className="flex gap-2">
              ...
              <p className="text-muted-foreground line-clamp-2">
                {t.rich('pages.feed.persons.new_activity', {
                  ...
                  roles: item.jobs.length
                    ? item.jobs.join(', ').toLowerCase()
...
const FeedPersonsItem = forwardRef<HTMLDivElement, FeedPersonsItemProps>(
  ({ className, ...props }, ref) => {
    return (
...
export { type FeedPersonsItemProps, FeedPersonsItem, FeedPersonsItemDefault };
