...
const RightPanelSocialFollows = () => {
  ...
  const fuse = useMemo(() => {
    ...
    return new Fuse(
      followees.pages.flatMap((page) => page.data),
      {
        keys: ['username', 'name'],
        threshold: 0.3,
      },
      ...
      {flattenedRequests.length ? (
        <>
          {flattenedRequests.map(({ createdAt, user }, i) => (
            <Card key={i} className={'flex flex-col rounded-xl bg-muted hover:bg-muted-hover p-1'}>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Button variant="outline" onClick={() => handleDeclineRequest(user.id)}>
