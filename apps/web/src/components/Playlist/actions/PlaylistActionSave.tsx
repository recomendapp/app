...
import {
  ...
  useUserPlaylistSaveMutation,
  useUserPlaylistUnsaveMutation,
} from '@libs/query-client';

interface PlaylistActionSaveProps extends React.ComponentProps<typeof Button> {
  playlistId: number;
  stopPropagation?: boolean;
}

const PlaylistActionSave = React.forwardRef<HTMLDivElement, PlaylistActionSaveProps>(
  ({ playlistId, stopPropagation = true, ...props }, ref) => {
    ...
    return (
      <TooltipBox
        tooltip={
          saved ? upperFirst(t('common.messages.delete')) : upperFirst(t('common.messages.save'))
        }
      >
        <Button
          onClick={saved ? handleUnsave : handleSave}
          disabled={
            isLoading || isError || saved === undefined || insertIsPending || deleteIsPending
          }
          size="icon"
          variant={'outline'}
          {...props}
