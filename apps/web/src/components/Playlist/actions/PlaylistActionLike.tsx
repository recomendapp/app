...
import {
  ...
  useUserPlaylistLikeMutation,
  useUserPlaylistUnlikeMutation,
} from '@libs/query-client';

interface PlaylistActionLikeProps extends React.ComponentProps<typeof Button> {
  playlistId: number;
  stopPropagation?: boolean;
}

const PlaylistActionLike = React.forwardRef<HTMLDivElement, PlaylistActionLikeProps>(
  ({ playlistId, stopPropagation = true, ...props }, ref) => {
    ...
    return (
      <TooltipBox
        tooltip={
          liked ? upperFirst(t('common.messages.delete')) : upperFirst(t('common.messages.save'))
        }
      >
        <Button
          onClick={liked ? handleUnsave : handleLike}
          disabled={
            isLoading || isError || liked === undefined || insertIsPending || deleteIsPending
          }
          size="icon"
          variant={'outline'}
          {...props}
