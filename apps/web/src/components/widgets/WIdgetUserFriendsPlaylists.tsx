...
export const WidgetUserFriendsPlaylists = ({ className }: React.HTMLAttributes<HTMLDivElement>) => {
  const { user } = useAuth();
  ...
  return (
    <div className={cn('@container/widget-user-friends-playlists space-y-4', className)}>
      <Button
        variant={'link'}
        className="p-0 w-fit font-semibold text-xl hover:text-primary hover:no-underline cursor-default"
      >
