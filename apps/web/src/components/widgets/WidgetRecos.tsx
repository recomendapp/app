...
import { AvatarGroup, AvatarGroupCount } from '@libs/ui/components/avatar';
...
export const WidgetRecos = ({ limit = ITEM_LIMIT, className }: WidgetRecosProps) => {
  const { user } = useAuth();
  ...
  return (
    <div className={cn('@container/widget-user-recos space-y-2', className)}>
      <Button variant={'link'} className="p-0 w-fit font-semibold text-xl" asChild>
        <Link href={'/collection/my-recos'}>{upperFirst(t('messages.reco_by_your_friends'))}</Link>
      </Button>
      <div className="grid grid-cols-2 @2xl/widget-user-recos:grid-cols-3 gap-4">
        {flattendRecos.map((item, index) => (
          <Wrapper
            key={index}
            {...(item.type === 'movie'
              ? { type: 'movie', media: item.media }
              : { type: 'tv_series', media: item.media })}
          >
            <AvatarGroup>
              {item.senders
                ?.slice(0, sendersShow)
                ...
                .map((item, i) => (
                  <div key={i} className="relative">
                    ...
                    {item?.comment ? (
                      <TextIcon
                        size={15}
                        className="absolute -top-1 -right-1 rounded-full bg-background text-accent-yellow p-1"
                      />
                    ) : null}
                  </div>
                ))}
...
const Wrapper = ({
  ...
  { type: 'movie'; media: MovieCompact } | { type: 'tv_series'; media: TvSeriesCompact }
>) => {
  switch (type) {
    case 'tv_series':
