...
export const WidgetUserFeed = ({ className }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  return (
    <div className={cn('@container/widget-user-feed space-y-4', className)}>
      <Button variant={'link'} className="p-0 w-fit font-semibold text-xl" asChild>
        <Link href={'/feed'}>{upperFirst(t('common.messages.latest_activity', { count: 0 }))}</Link>
      </Button>
      <div className="grid gap-2 grid-cols-1 @5xl/widget-user-feed:grid-cols-2">
        {feed.pages[0].data
          .slice(0, WIDGET_USER_FEED_LIMIT)
          .map((item, index) =>
