...
export const WidgetUserDiscovery = ({ className }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  const orderOptions = useMemo(
    (): { value: 'created_at' | 'followers_count'; label: string }[] => [
    ...
    <div className={cn('flex flex-col gap-4 overflow-hidden', className)}>
      <div className="flex items-center justify-between">
        ...
        <ButtonGroup>
          ...
          <Select
            onValueChange={(value) => setSortBy(value as 'created_at' | 'followers_count')}
            value={sortBy}
          >
      ...
      <ScrollArea className="h-96 gap-2">
        <div className="grid gap-2 h-full">
          ...
          {isFetching ? <Icons.loader className="mx-auto" /> : <div ref={ref} />}
        </div>
      </ScrollArea>
    </div>
  );
