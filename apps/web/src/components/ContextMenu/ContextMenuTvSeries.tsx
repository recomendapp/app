...
import {
  ...
  ContextMenuTrigger,
} from '@libs/ui/components/context-menu';
      ...
      <ContextMenuContent className="w-56">
        {items.map((group, fragindex) => (
          <Fragment key={fragindex}>
                ...
                <ContextMenuSub key={index}>
                  ...
                  <ContextMenuSubContent>
                    {item.submenu.map((subItem, subIndex) => (
                      <ContextMenuItem key={subIndex} className="gap-2" asChild>
                        <WithLink href={subItem.href}>{subItem.label}</WithLink>
                      </ContextMenuItem>
                    ))}
                  </ContextMenuSubContent>
                </ContextMenuSub>
              ) : (
                <ContextMenuItem key={index} className="gap-2" onClick={item.onClick} asChild>
            ...
            {fragindex < items.length - 1 && items[fragindex].length > 0 && (
              <ContextMenuSeparator />
            )}
          </Fragment>
        ))}
