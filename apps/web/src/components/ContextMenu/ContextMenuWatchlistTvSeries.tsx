      ...
      [
        {
          icon: Icons.comment,
          onClick: () =>
            watchlistItem && openModal(ModalUserWatchlistTvSeriesComment, { watchlistItem }),
          label: watchlistItem?.comment
            ? upperFirst(t('common.messages.edit_comment', { count: 1 }))
        ...
        {items.map((group, fragindex) => (
          <Fragment key={fragindex}>
            {group.map((item, index) => (
              <ContextMenuItem key={index} className="gap-2" onClick={item.onClick}>
