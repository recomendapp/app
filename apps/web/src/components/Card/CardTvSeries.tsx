...
interface CardTvSeriesProps extends React.ComponentProps<typeof Card> {
...
const CardTvSeriesDefault = React.forwardRef<HTMLDivElement, Omit<CardTvSeriesProps, 'variant'>>(
  (
    {
      className,
      tvSeries,
...
const CardTvSeriesPoster = React.forwardRef<HTMLDivElement, Omit<CardTvSeriesProps, 'variant'>>(
  (
    {
      className,
      tvSeries,
    ...
    return (
      <TooltipBox
        tooltip={`${tvSeries.name}${tvSeries.firstAirDate ? ` (${new Date(tvSeries.firstAirDate).getFullYear()})` : ''}`}
        side="top"
      >
        <Card
          ref={ref}
        ...
        >
          ...
          profile?.log?.isReviewed ? (
            <div className="absolute top-1 right-1 flex flex-col gap-1">
              {tvSeries.voteAverage ? (
                <IconMediaRating disableTooltip rating={tvSeries.voteAverage} />
              ) : null}
...
const CardTvSeriesRow = React.forwardRef<HTMLDivElement, Omit<CardTvSeriesProps, 'variant'>>(
  (
    {
      className,
      posterClassName,
      ...
      >
        ...
        <div className="flex items-center gap-4 justify-between w-full">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              {profile?.log?.rating && (
                <WithLink
                  href={
                    linked
                      ? `/@${profile?.user?.username}/tv-series/${tvSeries.slug ?? tvSeries.id}`
                      : undefined
                  }
            ...
            {tvSeries.createdBy && (
              <Credits credits={tvSeries.createdBy} linked={linked} className="line-clamp-2" />
            )}
...
const CardTvSeries = React.forwardRef<HTMLDivElement, CardTvSeriesProps>(
  (
    {
      className,
      tvSeries,
