...
interface CardPersonProps extends React.ComponentProps<typeof Card> {
    ...
    return (
      <WithLink href={person.url ?? undefined}>
        ...
        >
          <div
            className={cn(
              'relative h-full shrink-0 overflow-hidden aspect-square rounded-full',
              posterClassName,
            )}
          >
            <ImageWithFallback
...
const CardPersonRow = React.forwardRef<HTMLDivElement, Omit<CardPersonProps, 'variant'>>(
...
const CardPerson = React.forwardRef<HTMLDivElement, CardPersonProps>(
...
export { type CardPersonProps, CardPerson, CardPersonDefault, CardPersonRow };
