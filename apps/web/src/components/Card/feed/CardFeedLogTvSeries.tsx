...
const FeedActivity = ({ data, className }: { data: FeedItemLogTvSeries; className?: string }) => {
    ...
    <div className={cn('space-x-2', className)}>
      {data?.content.review ? (
        ...
        <>
          ...
          {data?.content.rating && (
            <IconMediaRating rating={data.content.rating} className="inline-flex" />
          )}
          {data?.content.isLiked && (
            <Icons.like size={24} className="text-background fill-accent-pink inline-flex" />
          )}
        </>
      )}
    </div>
...
export const CardFeedLogTvSeries = forwardRef<HTMLDivElement, CardFeedLogTvSeriesProps>(
  ({ data, ...props }, ref) => {
        ...
        <div className="flex flex-col gap-4 w-full">
          <div className="flex justify-between">
            ...
            <div className="flex items-center gap-1">
              ...
              <FeedActivity
                data={data}
                className="text-sm @md/feed-item:text-base text-muted-foreground"
              />
            </div>
            {data.createdAt && (
          ...
          <Link
            href={{
              pathname: `/tv-series/${data.content.tvSeries.slug || data.content.tvSeries.id}`,
            }}
            className="text-md @md/feed-item:text-xl space-x-1 line-clamp-2"
          >
            <span className="font-bold">{data.content.tvSeries?.name}</span>
            {/* DATE */}
            {data.content.tvSeries?.firstAirDate && (
              <sup>
                <DateOnlyYearTooltip
                  date={data.content.tvSeries.firstAirDate}
                  className="text-xs @md/feed-item:text-sm font-medium"
                />
              </sup>
            )}
          </Link>
            ...
            data.content.tvSeries.overview && (
              <p
                className={cn(
                  ...
                  !data.content.tvSeries.overview.length && 'text-muted-foreground',
                )}
              >
                {data.content.tvSeries.overview.length
                  ? data.content.tvSeries.overview
                  : upperFirst(t('common.messages.no_overview'))}
              </p>
            )
          )}
        </div>
