...
const FeedActivity = ({ data, className }: { data: FeedItemLogMovie; className?: string }) => {
...
export const CardFeedLogMovie = forwardRef<HTMLDivElement, CardFeedLogMovieProps>(
  ({ data, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col gap-4 w-full">
          ...
          <Link
            href={{
              pathname: `/film/${data.content.movie.slug || data.content.movie.id}`,
            }}
            className="text-md @md/feed-item:text-xl space-x-1 line-clamp-2"
          >
            <span className="font-bold">{data.content.movie?.title}</span>
            {/* DATE */}
            {data.content.movie?.releaseDate && (
              <sup>
                <DateOnlyYearTooltip
                  date={data.content.movie.releaseDate}
            ...
            data.content.movie.overview && (
              <p
                className={cn(
                  ...
                  !data.content.movie.overview.length && 'text-muted-foreground',
                )}
              >
                {data.content.movie.overview.length
                  ? data.content.movie.overview
