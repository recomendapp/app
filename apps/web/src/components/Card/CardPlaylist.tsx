...
interface CardPlaylistProps extends React.HTMLAttributes<HTMLDivElement> {
  variant?: 'default';
  playlist: Playlist;
...
const CardPlaylistDefault = React.forwardRef<HTMLDivElement, Omit<CardPlaylistProps, 'variant'>>(
  ({ className, playlist, owner, showItemCount, children, ...props }, ref) => {
    const t = useTranslations();
    return (
      <div ref={ref} className={cn('group border-none', className)} {...props}>
        <div className="p-0">
        ...
        <div className="p-0">
          <p className="line-clamp-2 wrap-break-word group-hover:text-primary/80">
            {playlist.title}
          </p>
          {owner && (
...
const CardPlaylist = React.forwardRef<HTMLDivElement, CardPlaylistProps>(
...
export { type CardPlaylistProps, CardPlaylist, CardPlaylistDefault };
