...
export const ModalLogMovieWatchedDates = ({
  ...
  const { mutateAsync: updateWatchedDate, isPending: isUpdatePending } =
    useMovieWatchedDateUpdateMutation();
  const { mutateAsync: deleteWatchedDate, isPending: isDeletePending } =
    useMovieWatchedDateDeleteMutation();
  ...
  return (
    <Modal open={props.open} onOpenChange={(open) => !open && closeModal(props.id)}>
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0 flex flex-col justify-center">
                      <Calendar
                        ...
                        onSelect={(newDate) =>
                          newDate &&
                          handleUpdateDate({ ...date, watchedDate: newDate.toISOString() })
                        }
      ...
      <ModalFooter>
        <Button
          variant="outline"
          onClick={() => handleInsertDate(new Date())}
