...
export const ModalShare = <T,>({
  ...
  const url = useMemo(() => `${location.origin}${path}`, [path]);
    ...
    <Modal open={props.open} onOpenChange={(open) => !open && closeModal(props.id)}>
      ...
      <ModalBody className="space-y-2">
        <div className="flex flex-col items-center gap-2">
          ...
          {canShareController && (
            <>
              <Button disabled={!canShareController} onClick={() => onShare(sharedControllerData)}>
        ...
        <div>
          <h3 className="text-lg font-semibold mb-2">{upperFirst(common('messages.via'))}</h3>
          <div className="flex gap-2">
            <TwitterShareButton url={url} via={siteConfig.socials.twitter.username}>
              <XIcon size={32} round />
            </TwitterShareButton>
