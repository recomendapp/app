...
export const ModalRecoSenders = ({ senders, ...props }: ModalRecosSendersProps) => {
      ...
      <ModalBody>
        <ScrollArea className="h-[40vh]">
          <div className="space-y-2">
            {senders?.map((item) => (
              <div key={item.user.id} className="bg-muted rounded-xl p-2 space-y-2">
                <CardUser user={item.user} variant="inline" />
                {item.comment && (
                  <div className="pl-8">
                    <div className="bg-background rounded-md p-2">{item.comment}</div>
                  </div>
                )}
              </div>
            ))}
