...
export function ProfileFollowersModal({ userId }: { userId: string }) {
          ...
          followers?.pages.map((page, i) =>
            page?.data.map((follower, index) => (
              <Link
                ...
                {...(i === followers.pages.length - 1 && index === page.data.length - 1
                  ? { ref: ref }
                  : {})}
              >
                ...
                <div>
                  ...
                  <p className="text-muted-foreground line-clamp-1">@{follower.username}</p>
                </div>
              </Link>
            )),
          )
