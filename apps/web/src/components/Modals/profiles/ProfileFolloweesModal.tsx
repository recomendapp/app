...
export function ProfileFolloweesModal({ userId }: { userId: string }) {
          ...
          following?.pages.map((page, i) =>
            page?.data.map((followee, index) => (
              <Link
                ...
                {...(i === following.pages.length - 1 && index === page.data.length - 1
                  ? { ref: ref }
                  : {})}
              >
                <Avatar className="h-10 w-10 shadow-2xl">
                  <AvatarImage src={followee.avatar ?? undefined} alt={followee.username} />
                ...
                <div>
                  ...
                  <p className="text-muted-foreground line-clamp-1">@{followee.username}</p>
                </div>
              </Link>
            )),
          )
