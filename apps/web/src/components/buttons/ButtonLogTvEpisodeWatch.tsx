...
interface ButtonLogTvEpisodeWatchProps extends React.ComponentProps<typeof Button> {
  tvSeriesId: number;
  seasonNumber: number;
...
>(
  (
    { tvSeriesId, seasonNumber, episodeNumber, stopPropagation = true, className, ...props },
    ref,
  ) => {
    const { user } = useAuth();
    ...
    const handleInsertActivity = React.useCallback(
    ...
    return (
      <TooltipBox
        tooltip={
          activity
            ? upperFirst(t('common.messages.remove_from_watched'))
      ...
      >
        <Button
          ...
          disabled={
            isLoading || isError || activity === undefined || isLogPending || isUnlogPending
          }
          size="icon"
          variant={'outline'}
          className={cn('rounded-full', activity ? 'bg-accent-blue!' : '', className)}
          {...props}
        >
