...
export default function ButtonUserReviewTvSeriesCommentLike({
  reviewId,
  commentId,
  parentId,
  likesCount,
  ...
  const { mutateAsync: like, isPending: isLikePending } = useReviewTvSeriesCommentLikeMutation({
    userId: user?.id,
    parentId,
  });
  const { mutateAsync: unlike, isPending: isUnlikePending } =
    useReviewTvSeriesCommentUnlikeMutation({
      userId: user?.id,
      parentId,
    });
