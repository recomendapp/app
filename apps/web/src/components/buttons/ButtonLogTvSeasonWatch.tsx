...
interface ButtonLogTvSeasonWatchProps extends React.ComponentProps<typeof Button> {
...
>(({ tvSeason, className, ...props }, ref) => {
  ...
  const handleInsertActivity = React.useCallback(
    async (status?: LogTvSeason['status']) => {
      await handleLog(
        {
          path: {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ref={ref}
          disabled={
            isLoading || isError || activity === undefined || isInsertPending || isDeletePending
          }
          size="icon"
          variant={'outline'}
          className={cn(
            ...
            <DropdownMenuItem
              onClick={() => {
                createConfirmModal({
                  ...
                  description: t('components.tv_series.season.actions.watch.complete.description', {
                    seasonNumber: tvSeason.seasonNumber,
                  }),
