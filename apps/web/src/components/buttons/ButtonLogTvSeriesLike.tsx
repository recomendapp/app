...
interface ButtonLogTvSeriesLikeProps extends React.ComponentProps<typeof Button> {
...
>(({ tvSeries, stopPropagation = true, className, ...props }, ref) => {
  ...
  return (
    <TooltipBox
      tooltip={
        activity?.isLiked
          ? upperFirst(t('common.messages.remove_from_heart_picks'))
          : upperFirst(t('common.messages.add_to_heart_picks'))
      }
    >
      <Button
        ref={ref}
        ...
        size="icon"
        variant={'outline'}
        className={cn('rounded-full', activity?.isLiked ? 'bg-accent-pink!' : '', className)}
        {...props}
      >
        {isLoading || activity === undefined ? (
          ...
          <Icons.like className={`${activity?.isLiked ? 'fill-foreground' : ''}`} />
        )}
      </Button>
    </TooltipBox>
  );
