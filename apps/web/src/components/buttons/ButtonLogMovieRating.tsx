...
interface ButtonLogMovieRatingProps extends React.ComponentProps<typeof Button> {
  movieId: number;
  stopPropagation?: boolean;
}

const ButtonLogMovieRating = React.forwardRef<
  React.ComponentRef<typeof Button>,
  ButtonLogMovieRatingProps
>(({ movieId, stopPropagation = true, className, ...props }, ref) => {
  ...
  const handleUnrate = React.useCallback(
    async (e: React.MouseEvent<HTMLButtonElement>) => {
      stopPropagation && e.stopPropagation();
      await handleLog(
        {
  ...
  return (
    <Dialog>
      <TooltipBox
        tooltip={
          activity?.rating
            ? upperFirst(t('common.messages.edit_rating'))
      ...
      >
        <DialogTrigger asChild>
          <Button
            ...
            className={cn(
              activity?.rating
                ? 'bg-background! border-accent-yellow! text-accent-yellow! border-2'
                : 'rounded-full',
              className,
            )}
            {...props}
      ...
      <DialogContent>
        ...
        <DialogFooter className="flex flex-col justify-center">
          <DialogClose asChild>
            <Button onClick={async (e) => handleRate(e)}>
...
ButtonLogMovieRating.displayName = 'ButtonLogMovieRating';
...
export default ButtonLogMovieRating;
