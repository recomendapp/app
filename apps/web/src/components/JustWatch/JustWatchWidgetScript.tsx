...
export function JustWatchWidget({
  ...
  const resetAndReloadWidget = React.useCallback(() => {
    ...
    // @ts-expect-error: JustWatch is an external global variable injected by their script
    if (typeof window !== 'undefined' && window['JustWatch']) {
      // @ts-expect-error: We need to force delete this external variable to reload the widget
  ...
  React.useEffect(() => {
    ...
    return () => {
      // @ts-expect-error: Cleaning up external global variable on unmount
      delete window['JustWatch'];
  ...
  return (
    <div className={cn('overflow-hidden relative', className)}>
      ...
      >
        ...
        <Icons.eye
          size={15}
          className="inline-block ml-1 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity"
        />
      </div>
      <div>
        <div
