...
export const UIProvider = ({
  ...
  const toggleSidebar = useCallback(() => {
    return isMobile ? setSidebarOpenMobile((open) => !open) : sidebarOpenChange(!sidebarOpen);
  }, [isMobile, sidebarOpen, sidebarOpenChange]);
  // *========== END SIDEBAR ==========*

  // *========== START RIGHTPANEL ==========*
  const [rightPanelOpen, setRightPanelOpen] = useState(user ? cookieRightPanelOpen : false);
  const [rightPanelOpenMobile, setRightPanelOpenMobile] = useState(false);
  const [rightPanel, setRightPanel] = useState<RightPanel<any> | null>(
    user ? RightPanelSocial : null,
  );
  ...
  const toggleRightPanel = useCallback(
    (open?: boolean) => {
      const newState =
        open !== undefined ? open : isMobile ? !rightPanelOpenMobile : !rightPanelOpen;
  ...
  const toggleRightPanelContent = <P,>(content: RightPanel) => {
    ...
    const isSameContent =
      rightPanel?.title === content.title &&
      rightPanel?.component === content.component &&
      JSON.stringify(rightPanel?.props) === JSON.stringify(content.props);
    if (isSameContent) {
      toggleRightPanel();
    } else {
